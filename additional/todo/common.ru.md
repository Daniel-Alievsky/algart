...
Надо ***попробовать*** использовать StandardOpenOption.DELETE_ON_CLOSE в сочетании с FileChannel.open для временных файлов

Еще есть ***Arena*** в Java 22: см. MappedFileExample в contrib (собран Gemini), 
```
try (Arena arena = Arena.ofConfined()) {
MemorySegment segment = fileChannel.map(MapMode.READ_ONLY, 0, fileSize, arena);
...
}
```
Может быть, это вообще полностью решает вопрос сверхбольшого массива - достаточно отвести огромный блок на диске. И заодно можно будет создать большие массивы в памяти вместо ByteBuffer. Хотя тут возможны нюансы, надо разбираться.

 В следующей крупной версии желательно перенести проект на GitHub, сохранив в виде архива проект на BitBucket и поставив там ссылку. _Сделано_ 
